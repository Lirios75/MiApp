...
⏸️ CHECKPOINT — Sesión 4 EN CONSTRUCCIÓN (certificación PENDIENTE), 11ª pasada del revisor en curso para onboarding y paywall. Commit a363b0a pusheado.
DECISIÓN DEL USUARIO (tras presentarle la disyuntiva de la ronda 10): eligió "terminar de pulirlo" — no aceptar el estado actual del onboarding, sino cerrar los 2 defectos puntuales que quedaban (celebración vacía + VistaSemaforo repetida idéntica) antes de una nueva pasada. Implementado: `VistaSemaforo` ahora varía su texto por paso (`contexto: dolor/momento/intento/meta`) y dejó de invitar a tocar algo que no responde; la celebración ahora extiende `<SemanaPreview diaCompletado>` marcando el día de hoy, cerrando el vacío más grande del onboarding.
PAYWALL — 5 rondas con craft (16-19/20) y copy (19/20) pasando con margen de forma estable; usabilidad muy cerca del gate (32→33→32→33→32→33/40). Ronda 10 (reintentada tras fallo de rate-limit) encontró 3 defectos concretos y verificables contra el propio código/FICHA-ARTE: el X saltaba de lado entre fases → unificado a la derecha; 8 tamaños de fuente vs. la escala de 4 niveles declarada → consolidados; ConfirmSalir sin trap de foco (Tab escapaba al fondo) → ahora cicla entre sus 2 botones.
NOTA METODOLÓGICA (sigue vigente): commitear SIEMPRE antes de lanzar el revisor-visual; si un revisor-visual falla por rate-limit, no computar la falla como una ronda real.
Siguiente acción exacta: leer los veredictos de la 11ª pasada. Si el paywall pasa el gate, cerrarlo. Si el onboarding sigue NO LISTA pero sin defectos nuevos accionables (solo refinamiento subjetivo), presentar de nuevo la disyuntiva en vez de seguir indefinidamente.
...
- veredicto onboarding PENDIENTE de certificación formal (Sesión 4, en construcción) — última pasada leída (11ª): usabilidad 34/40, craft 14/20 — gate exige ≥36/40 y ≥16/20, sigue NO LISTA tras 11 rondas pero MUY cerca en usabilidad (subió de 28→34 tras cerrar los 2 defectos puntuales de la 10ª ronda). La 11ª pasada encontró 2 defectos nuevos, ambos verificables contra la propia FICHA-ARTE.md (no gusto subjetivo): (a) los checkmarks animados usaban bounce 0.4-0.5 quien la ficha pide "casi sin rebote (0-0.1)" → corregido a 0.15; (b) la celebración seguía siendo, proporcionalmente, el tramo más vacío del funnel pese al tracker semanal → se agregó una frase de refuerzo bajo el tracker. Se relanzó una 12ª pasada, resultado aún no leído. No declarar esta pantalla "lista" hasta que el veredicto diga LISTA o el usuario acepte una disyuntiva explícita.
- veredicto paywall PENDIENTE de certificación formal (Sesión 4, en construcción) — última pasada leída (10ª): usabilidad 33/40 (gate ≥36/40, muy cerca), craft 19/20 y copy 19/20 YA PASAN CON MARGEN desde hace varias rondas. Se corrigieron los 3 defectos concretos de esa pasada (X saltaba de lado entre fases → unificado a la derecha; 8 tamaños de fuente vs. la escala de 4 niveles de FICHA-ARTE → consolidados; ConfirmSalir sin trap de foco → ahora cicla Tab entre sus 2 botones) y se relanzó una 11ª pasada — AÚN EN CURSO al momento de este checkpoint (el archivo de veredicto todavía muestra el contenido de la 10ª ronda, confirmado sin diff contra el commit a363b0a). No declarar esta pantalla "lista" hasta que el veredicto diga LISTA o el usuario acepte una disyuntiva explícita.

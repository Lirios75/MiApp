...
⏸️ CHECKPOINT — Sesión 4 EN CONSTRUCCIÓN (certificación PENDIENTE), 7ª pasada del revisor en curso para onboarding y paywall (commit d293f62 pusheado). Hallazgo clave de la ronda 6 (ya corregido y verificado): el anillo/mesh de fondo NO era un límite del sandbox headless (como se creía desde la landing) sino un bug real de CSS de stacking-context — el contenedor `position:relative` con `z-index:auto` no crea contexto propio, así que sus hijos `-z-10` quedaban tapados por el propio `bg-[var(--bg)]` del contenedor. Fix: `isolate` en el contenedor + `position:fixed` en los decorativos. Mismo fix pendiente de aplicar a la landing (Hero `EcoAnillo`) si se vuelve a tocar esa pantalla — ver Problemas conocidos.
Fixes de la ronda 6 (verificados en código y screenshot, no solo descritos): paso 0 del onboarding sin X de salida → agregada; `HoldButton` con reduced-motion disparaba el compromiso en un solo tap (rompía el ritual) → ahora exige sostener vía setTimeout; espacio muerto en preguntas cortas y en el paso de compromiso → `justify-center` en el contenedor + mesh/anillo agrandados (420px) y animados con pulso sutil; garantía del paywall sin nombre completo vs. landing → alineada ("...sin culpa"); bug real en `lib/pricing.ts` (`PLAN_MENSUAL.montoCobro` con "/mes" duplicado en la timeline) → corregido y confirmado con inspección de DOM; CTA con timeout mudo → ahora muestra mensaje de error visible; plan elegido se descartaba al ir a /entrar → se persiste en localStorage; card del plan recomendado sin el hairline degradé (gate binario de conversión) → agregado `<Hairline emphasis>`; fondo decorativo duplicado entre onboarding y paywall (mismo bug arreglándose dos veces) → extraído a `<FunnelFondo>` compartido en `components/funnel/ui.tsx`.
tsc ✓ build ✓ · recapturado el flujo completo, incluida la captura de "cargando" del paywall que en la ronda 6 estaba mal (mostraba por error la celebración del onboarding).
NOTA METODOLÓGICA para próximas rondas: el subagente revisor-visual puede tardar minutos y devolver un veredicto que analiza el código ANTERIOR a un commit reciente si se lanza antes de commitear — pasó en la ronda 6 (dos pasadas leyeron código viejo). Regla: commitear ANTES de lanzar el revisor, no después.
Siguiente acción exacta: leer los veredictos de la 7ª pasada. Si alguna sigue NO LISTA por refinamiento fino sin defecto concreto nuevo, presentarle al usuario la disyuntiva (seguir iterando vs. aceptar y avanzar a Sesión 5). Si LISTA en ambas, cerrar Sesión 4 (puertas de etapa) y commit+push.
...
- veredicto onboarding PENDIENTE de certificación formal (Sesión 4, en construcción) — última pasada leída (6ª): usabilidad 31/40, craft 13/20 — gate exige ≥36/40 y ≥16/20, sigue NO LISTA. Desde esa pasada se corrigieron defectos concretos (X de salida en paso 0, HoldButton con reduced-motion, espacio muerto vía justify-center + fondo agrandado/animado) y se relanzó una 7ª pasada — resultado aún no leído al momento de este checkpoint. No declarar esta pantalla "lista" hasta que el veredicto diga LISTA o el usuario acepte una disyuntiva explícita.
- veredicto paywall PENDIENTE de certificación formal (Sesión 4, en construcción) — última pasada leída (6ª): usabilidad 32-33/40, craft 14/20 (copy 19/20 sí pasa) — gate exige ≥36/40 y ≥16/20, sigue NO LISTA. Desde esa pasada se corrigieron defectos concretos (hairline en plan recomendado, garantía alineada con la landing, bug de "/mes" duplicado en lib/pricing.ts, CTA con mensaje de error visible, plan persistido, fondo compartido vía FunnelFondo) y se relanzó una 7ª pasada — resultado aún no leído al momento de este checkpoint. No declarar esta pantalla "lista" hasta que el veredicto diga LISTA o el usuario acepte una disyuntiva explícita.

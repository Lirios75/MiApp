...
⏸️ CHECKPOINT — Sesión 4 EN CONSTRUCCIÓN (certificación PENDIENTE), 9ª pasada del revisor en curso para onboarding y paywall (commit 4b0793d pusheado). Progreso medido ronda a ronda: onboarding 29→31→30→27/40 usabilidad (oscilando 4 rondas sin mejora limpia) y 13→14→12→14/20 craft; paywall 32→33→32→33/40 usabilidad (craft y copy YA PASAN desde la ronda 7, cayeron a 14/20 en la ronda 8 SOLO por el fondo plano de la fase de carga — ya corregido esta ronda).
Hallazgos estructurales ya corregidos y verificados (no solo descritos): (a) ronda 6 — el anillo/mesh de fondo no era un límite del sandbox headless sino un bug real de stacking-context (`isolate` + `position:fixed`, extraído a `<FunnelFondo>` compartido); (b) ronda 7 — `HoldButton` sin soporte de teclado bloqueaba el funnel a usuarios de solo-teclado, y el `justify-center` de la ronda 6 era matemáticamente equivalente a centrar (no resolvía el vacío); (c) ronda 8 — `ConfirmSalir` afirmaba una pérdida de datos que no ocurre en el paywall (las respuestas ya están en localStorage) y `FunnelHeader`/`HoldButton` carecían de ARIA (progressbar, aria-live).
DECISIÓN DE PROCESO tras 8 rondas: el defecto de "espacio vacío" en los pasos cortos del onboarding se ha reportado en 4 ubicaciones distintas (arriba, abajo, "casi la mitad del viewport") pese a 3 intentos genuinamente distintos (centrado, top-anchor, fondo agrandado+animado) — patrón de oscilación sin convergencia, no de mejora bloqueada por un bug concreto. Si la 9ª pasada no aporta un defecto NUEVO y accionable en ese eje, se presentará al usuario la disyuntiva para onboarding (seguir iterando vs. aceptar y avanzar), igual que se hizo con la landing en la Sesión 3 — el paywall, en cambio, sigue mostrando defectos concretos ronda a ronda y está a un solo eje del gate, así que continúa su propio curso independientemente de lo que se decida para onboarding.
NOTA METODOLÓGICA (sigue vigente): commitear SIEMPRE antes de lanzar el revisor-visual.
Siguiente acción exacta: leer los veredictos de la 9ª pasada y decidir cierre de ronda para cada pantalla por separado.

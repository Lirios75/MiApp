...
⏸️ CHECKPOINT — Sesión 4 EN CONSTRUCCIÓN, 3ª pasada del revisor en curso para onboarding y paywall. Progreso: 2ª pasada dio onboarding 30/40·13/20 (NO LISTA) y paywall 32/40·craft 16/20 ya pasa·copy 19/20 ya pasa (NO LISTA solo por usabilidad). Se corrigieron: onboarding (logo estático sin pinta de spinner via `RespiraMark estatico`, fondo con profundidad + eco del anillo en FunnelScreen para el vacío inferior, línea de contexto en paso 0) y paywall (whileTap en las cards de plan, shadow-2 en la card seleccionada para 2º nivel de elevación). 3 defectos de paywall quedaron aceptados a propósito (FICHA-AVATAR sin aprobar — pendiente de negocio ya rastreado; sin manejo de error de checkout — Sesión 6; X y "Ahora no" al mismo destino — no hay app limitada todavía). tsc ✓ build ✓ · recapturado y verificado visualmente. / Siguiente acción exacta: leer los veredictos de la 3ª pasada; si LISTA en ambas, cerrar Sesión 4 (actualizar puertas de etapa) y commit+push; si no, evaluar si conviene seguir iterando o aceptar un techo similar al de la landing (aprobación del usuario en sustancia).

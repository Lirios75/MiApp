...
⏸️ CHECKPOINT — Sesión 4 EN CONSTRUCCIÓN (certificación PENDIENTE), 10ª pasada del revisor en curso para onboarding y paywall (commit a7aabc8 pusheado). Progreso medido ronda a ronda: onboarding 29→31→30→27→28/40 usabilidad y 13→14→12→14/20 craft (aún sin gate, pero la ronda 9 identificó causa raíz nueva — ver abajo); paywall 32→33→32→33→32/40 usabilidad con craft (16/20) y copy (19/20) YA PASANDO desde la ronda 7 de forma estable.
GIRO IMPORTANTE en la ronda 9 (onboarding): tras 4 rondas reposicionando/agrandando el mismo elemento decorativo sin resolver el "espacio vacío", el revisor determinó EXPLÍCITAMENTE que es un defecto real con causa raíz de FALTA DE CONTENIDO, no de posición — pidió una solución cualitativamente distinta. Implementado esta ronda: `<VistaSemaforo>` (preview del mecanismo tranquilo/alerta, verde/naranja de FICHA-ARTE) bajo las opciones de los pasos 0/2/3/5, y `<SemanaPreview>` (7 días L-D vacíos) bajo el HoldButton del paso de compromiso — contenido funcional, no decoración. Resultado pendiente de leer en la 10ª pasada.
Hallazgos ya corregidos y verificados en rondas previas (no solo descritos): (a) ronda 6 — bug real de stacking-context en el fondo decorativo (`isolate`+`position:fixed`, extraído a `<FunnelFondo>` compartido); (b) ronda 7 — `HoldButton` sin teclado bloqueaba el funnel a usuarios de solo-teclado; (c) ronda 8 — `ConfirmSalir` afirmaba una pérdida de datos falsa en el paywall, `FunnelHeader`/`HoldButton` sin ARIA; (d) ronda 9 (paywall) — `ConfirmSalir` ahora con `role="dialog"`/`aria-modal`/foco automático/Escape (requirió convertir `FunnelButton` a `forwardRef`), titular con `whitespace-nowrap` para no partir "Semáforo del Gasto", error del CTA con `aria-live`.
NOTA METODOLÓGICA (sigue vigente): commitear SIEMPRE antes de lanzar el revisor-visual.
Siguiente acción exacta: leer los veredictos de la 10ª pasada. Paywall lleva 4 rondas con craft/copy estables y solo usabilidad pendiente — si esta pasada no aporta defectos nuevos y accionables, evaluar disyuntiva. Onboarding: evaluar si el contenido funcional nuevo (VistaSemaforo/SemanaPreview) resolvió genuinamente el eje de profundidad/encaje o si sigue fallando pese al enfoque distinto.

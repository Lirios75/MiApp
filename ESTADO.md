...
⏸️ CHECKPOINT — Sesión 4 CASI CERRADA. PAYWALL CERTIFICADO ✅ (37/40 · 20/20 · 19/20, 12ª pasada, 2026-09-18). ONBOARDING en su 14ª pasada del revisor, commit e747bef pusheado.
...
ONBOARDING: 13 rondas, última leída 31/40 usabilidad · 15/20 craft (gate ≥36/40, ≥16/20). El defecto TOP de la ronda 13 fue una condición de carrera real: el fix de ronda 12 quitó "Atrás" durante la celebración pero el LOGO seguía siendo un `<a href="/">` navegable sin confirmación en esos mismos 700ms — mismo bug, otro lugar, peor consecuencia. Corregido con `logoBloqueado` (FunnelHeader/FunnelScreen) — durante la celebración ni Atrás ni el logo pueden escapar sin pasar por la X con confirmación. También se corrigió el hex desactualizado en FICHA-ARTE.md que causaba un falso positivo de contraste en 2 rondas seguidas. Se relanzó una 14ª pasada — si esta ronda no aporta nada nuevo y accionable (sería la primera vez en 14 rondas), corresponde presentarle al usuario la disyuntiva en vez de seguir indefinidamente.
...
Siguiente acción exacta: leer el veredicto de la 14ª pasada de onboarding.

...
⏸️ CHECKPOINT — Sesión 4 CASI CERRADA. PAYWALL CERTIFICADO ✅ (37/40 · 20/20 · 19/20, 12ª pasada, 2026-09-18 — re-verificado tras cambios compartidos en ui.tsx sin impacto, ver docs/revisiones/paywall-veredicto.md). ONBOARDING en su 15ª pasada del revisor, commit 90128b0 pusheado.
ONBOARDING: 14 rondas, última leída 25/40 usabilidad · 14/20 craft (gate ≥36/40, ≥16/20) — la nota más baja de toda la racha, por un bug de arquitectura real y grave: `comprometerse()` disparaba un `setTimeout` de navegación a /paywall sin guardar su id ni cancelarlo nunca — si el usuario abría la confirmación de salida dentro de esos 700ms y elegía CUALQUIERA de las 2 opciones, el timer seguía corriendo y lo empujaba al paywall segundos después de todas formas, pisoteando su decisión. Corregido con un `useRef` que guarda el id y lo cancela en `salir()`; el mismo patrón de timer-sin-cleanup se corrigió también en `OptionChip` por consistencia. Verificado con una prueba automatizada end-to-end (hold-to-commit → X → "Seguir aquí" → esperar >700ms → confirma que la URL sigue en /onboarding) antes de relanzar el revisor. También se centró el paso 6 pre-hold (le faltaba el flex-1+justify-center que sí tienen los pasos 1 y 4). Se relanzó una 15ª pasada.
NOTA METODOLÓGICA (sigue vigente): commitear SIEMPRE antes de lanzar el revisor-visual; si un revisor-visual falla por rate-limit, no computar la falla como una ronda real. Nueva: verificar los fixes de "control y libertad" (timers, navegación) con una prueba automatizada end-to-end antes de relanzar el revisor, no solo leer el código.
Siguiente acción exacta: leer el veredicto de la 15ª pasada de onboarding.

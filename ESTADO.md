...
⏸️ CHECKPOINT — Sesión 4 EN CONSTRUCCIÓN (certificación PENDIENTE), 6ª pasada del revisor en curso para onboarding y paywall. Ronda 5 (onboarding): 29/40·13/20 NO LISTA, con un hallazgo importante: el anillo/mesh de fondo decorativo NO era un límite del sandbox headless (como se creía desde la landing) sino un bug real de CSS — el contenedor tenía `position:relative` con `z-index:auto` (no crea contexto de apilamiento), así que sus hijos `-z-10` quedaban tapados por el propio fondo del contenedor. Fix aplicado y VERIFICADO visualmente (screenshot con el anillo ya visible): se agregó `isolate` al contenedor + `position:fixed` en los decorativos, en `components/funnel/ui.tsx` (FunnelScreen) y `app/paywall/page.tsx`. Mismo fix pendiente de aplicar a la landing (Hero `EcoAnillo`) si se vuelve a tocar esa pantalla — ver nota en Problemas conocidos. Otros fixes de esta ronda: `ConfirmSalir` (modal propio) reemplaza `window.confirm()` en TODOS los pasos del onboarding (antes solo en paywall), celebración breve (~700ms, check animado) en el hito de compromiso antes de navegar a /paywall. tsc ✓ build ✓ · recapturado con flujo completo (7 pasos + modal + celebración + paywall). / Siguiente acción exacta: leer los veredictos de la 6ª pasada (en curso, corriendo en paralelo para onboarding y paywall). Si alguna sigue NO LISTA por refinamiento fino sin defecto concreto nuevo, presentarle al usuario la disyuntiva (seguir iterando vs. aceptar y avanzar a Sesión 5). Si LISTA en ambas, cerrar Sesión 4 (puertas de etapa) y commit+push.
